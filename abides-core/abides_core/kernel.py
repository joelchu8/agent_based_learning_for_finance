...
class Kernel:
    ...
    def __init__(
        ...
        self.agent_types: Optional[List[str]] = agent_types
    ...
    def write_log(
        self, sender_id: int, df_log: Union[pd.DataFrame, Dict[str, Any]], filename: Optional[str] = None
    ) -> None:
        """
        ...
        Arguments:
            sender_id: The ID of the agent making the call.
            df_log: dataframe for dict representation of the log that contains all the events logged during the simulation.
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        suffix = "bz2" if isinstance(df_log, pd.DataFrame) else "pkl" if isinstance(df_log, dict) else "null"
        if filename:
            file = "{}.{}".format(filename, suffix)
        else:
            file = "{}.{}".format(self.agents[sender_id].name.replace(" ", ""), suffix)

        if isinstance(df_log, pd.DataFrame):
            df_log.to_pickle(os.path.join(path, file), compression="bz2")
        elif isinstance(df_log, dict):
            with open(os.path.join(path, file), 'wb') as handle:
                pickle.dump(df_log, handle, protocol=pickle.HIGHEST_PROTOCOL)
        else:
            raise TypeError("Log must be a pandas DataFrame or a dictionary")
