...
class AdaptiveMarketMakerAgent(TradingAgent):
    ...
    def __init__(
        ...
        window_size: Union[float, str] = "adaptive",
        pov: float = 0.025, # fraction of transacted volume placed at each price level
        num_ticks: int = 3,
        wake_up_freq: str = '1S',  # 1 second
        min_order_size: int = 20,
        level_spacing: float = 5,
        cancel_limit_delay: int = 50, # 50 nanoseconds
        skew_beta=0,
        price_skew_param: Optional[int] = None,
        ...
        self.window_size: Union[float, str] = self.validate_window_size(
            window_size
        ...
        self.arrival_rate: NanosecondTime = str_to_ns(self.wake_up_freq)

        self.subscribe: bool = subscribe  # Flag to determine whether to subscribe to data or use polling mechanism
        self.subscribe_freq: str = subscribe_freq  # Frequency at which to receive market updates
        # in subscribe mode
        ...
        self.backstop_quantity: int = backstop_quantity  # order quantity to place at outside order level, to prevent liquidity dropouts. If None then place same as at other levels.
        ...
        self.last_spread: float = INITIAL_SPREAD_VALUE  # last observed spread moving average
    ...
    def validate_window_size(self, window_size: Union[float, str]) -> Optional[int]:
    ...
    def wakeup(self, current_time: NanosecondTime):
        ...
        if self.subscribe and not self.subscription_requested:
            ...
            # The "get_transacted_volume" method send a "QueryTransactedVolMsg" message to the exchange
        ...
        if not self.subscribe:
            ...
            if (isinstance(message, QuerySpreadResponseMsg)
                ...
                if bid and ask:
                    ...
                    self.state["AWAITING_SPREAD"] = False  # TODO:use last mid price and spread?

            if (
                self.state["AWAITING_SPREAD"] is False
                and self.state["AWAITING_TRANSACTED_VOLUME"] is False
            ):
                # self.cancelOrder_workflow() # Cancellation is done in the wakeup stage.
                if mid is not None:
                    self.place_orders(mid)
                self.state = self.initialise_state()
            ...
            if (
                isinstance(message, L2DataMsg)
                and self.state["AWAITING_MARKET_DATA"] is True
            ):
                # print("298:", self.state["AWAITING_MARKET_DATA"])
                bid = (
                ...
                if bid and ask:
                    ...
                    logger.debug("SPREAD MISSING at time {}", current_time)
                    self.state["AWAITING_MARKET_DATA"] = False # TDOO:use last mid price and spread?
                # print("321:", self.state["AWAITING_MARKET_DATA"])

            if (
                self.state["AWAITING_MARKET_DATA"] is False
                and self.state["AWAITING_TRANSACTED_VOLUME"] is False # CANNOT test mid None here!
            ):
                # print("debug: Placing orders")
                self.cancelOrder_workflow()
                self.delay(self.cancel_limit_delay)
                if mid is not None:
                    self.place_orders(mid)
                self.state = self.initialise_state()
            # print("333:", mid, self.state["AWAITING_MARKET_DATA"], self.state["AWAITING_TRANSACTED_VOLUME"])
    ...
    def compute_orders_to_place(self, mid: int) -> Tuple[List[int], List[int]]:
        ...
        if self.price_skew_param is None:
            ...
            if (buy_transacted_volume == 0) and (sell_transacted_volume == 0):
                ...
                trade_imbalance = (2 * buy_transacted_volume / (buy_transacted_volume + sell_transacted_volume)) - 1
