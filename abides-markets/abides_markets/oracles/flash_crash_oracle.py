...
class FlashCrashOracle(Oracle):
    def __init__(self, symbol: str, mkt_open: NanosecondTime, S0: int = 1e5, mu: float=3e-5, sigma: float = 0.0335, bounds: int = 400) -> None:
        ...
        self.higher = self.open_price + bounds
        self.lower = self.open_price - bounds
        ...
        logger.debug("FlashCrashOracle initialized")
    ...
    def observe_price(self, symbol: str, current_time: NanosecondTime, random_state: np.random.RandomState) -> int:
        ...
        # We bound the fundamental price in the event of a flash crash
        ...
        self.last_price = max(self.lower, min(self.last_price, self.higher))
